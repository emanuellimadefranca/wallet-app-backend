...
router.post("/", (req, res) => {
  try {
    ...
    if (name.length < 3) {
      return res
        .status(400)
        .json({ error: "Nome deve ter no minimo 3 Caracteres" });
    }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (!deleteResponse.rows[0]) {
      return res.status(400).json({ error: "Categoria não deletada" });
    }
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (!updateResponse.rows[0]) {
      return res.status(400).json({ error: "Categoria não atualizada" });
    }
